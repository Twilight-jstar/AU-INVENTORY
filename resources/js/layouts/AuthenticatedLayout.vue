...
const isRouteActive = (activePattern: string) => {
    try { return route().current(activePattern); } catch (e) { return false; }
};
                ...
                <div v-for="group in filteredGroups" :key="group.label" class="space-y-1">
                    ...
                    <div class="space-y-0.5">
                        <Link 
                            ...
                            :class="isRouteActive(item.active) ? 'bg-white/15 text-white border border-white/10' : 'text-purple-100/70 hover:bg-white/5 hover:text-white'"
                        >
