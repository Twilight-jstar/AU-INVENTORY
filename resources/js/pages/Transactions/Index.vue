...
<template>
    ...
    <AuthenticatedLayout>
        <div class="max-w-[1600px] mx-auto p-4 space-y-4">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div class="flex items-center gap-2">
                    <button v-if="activeTab === 'in' && startDate" @click="exportDailyInReport" class="px-4 py-2 bg-emerald-600 text-white text-[10px] font-black rounded-lg hover:bg-emerald-700 uppercase flex items-center gap-2 transition-all shadow-md shadow-emerald-100"><Download class="w-3.5 h-3.5" /> Export Daily In</button>
                    <button v-if="filterDept" @click="exportDepartmentReport" class="px-4 py-2 bg-blue-600 text-white text-[10px] font-black rounded-lg hover:bg-blue-700 uppercase flex items-center gap-2 transition-all shadow-md shadow-blue-100"><Download class="w-3.5 h-3.5" /> Export {{ filterDept }} Report</button>
                    <Link :href="route('web.transactions.stock-in')" class="bg-emerald-600 text-white px-4 py-2 text-[10px] font-black rounded-lg uppercase tracking-widest flex items-center gap-2 hover:bg-emerald-700 transition-all shadow-md shadow-emerald-100"><PackagePlus class="w-3.5 h-3.5" /> Stock In</Link>
                    <Link :href="route('web.transactions.stock-out')" class="bg-slate-900 text-white px-4 py-2 text-[10px] font-black rounded-lg uppercase tracking-widest flex items-center gap-2 hover:bg-slate-800 transition-all shadow-md shadow-slate-200"><PackageMinus class="w-3.5 h-3.5" /> Stock Out</Link>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-6 gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                ...
                <div class="md:col-span-2 flex gap-2 items-center">
                    <div class="flex-1 relative"><Calendar class="absolute left-2.5 top-2.5 w-3 h-3 text-slate-400" /><input type="date" v-model="startDate" class="w-full h-9 pl-8 pr-2 bg-white border-slate-200 rounded-lg text-[10px] font-bold focus:ring-slate-900"></div>
                    <div class="flex-1 relative"><Calendar class="absolute left-2.5 top-2.5 w-3 h-3 text-slate-400" /><input type="date" v-model="endDate" class="w-full h-9 pl-8 pr-2 bg-white border-slate-200 rounded-lg text-[10px] font-bold focus:ring-slate-900"></div>
                    <button @click="resetFilters" class="p-2 text-slate-300 hover:text-red-500 transition-colors"><XCircle class="w-5 h-5" /></button>
                </div>
            </div>
                            ...
                            <tr v-for="trx in filteredTransactions" :key="trx.id" class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="py-3 px-2">
                                    <div class="flex flex-col leading-tight max-w-[250px]">
                                        <span class="font-bold text-slate-900 uppercase text-[12px] truncate">{{ trx.item?.name || 'Unknown' }}</span>
                                        <span class="text-[10px] font-mono text-slate-400 tracking-tighter">{{ trx.item?.product_code }}</span>
                                    </div>
                                </td>
                                <td class="py-3 px-2">
                                    <span :class="trx.type === 'In' ? 'text-emerald-600 bg-emerald-50 border-emerald-100' : 'text-slate-600 bg-slate-50 border-slate-200'" class="text-[9px] font-black uppercase px-2 py-0.5 rounded border leading-none inline-block tracking-tighter">
                                        {{ trx.department || (trx.type === 'In' ? 'INBOUND' : 'GENERAL') }}
                                    </span>
                                </td>
                                <td class="py-3 px-2">
                                    <span class="text-[11px] text-slate-700 font-bold uppercase truncate block tracking-tight">{{ trx.received_by || trx.released_to || 'System' }}</span>
                                </td>
                                <td class="py-3 px-2 text-center">
                                    <span :class="trx.type === 'In' ? 'text-emerald-600' : 'text-purple-600'" class="font-black text-[12px]">{{ trx.type === 'In' ? '+' : '-' }}{{ trx.quantity }}</span>
                                </td>
                                <td class="py-3 px-4 text-right">
                                    <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button @click="openViewModal(trx)" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg"><Eye class="w-4 h-4" /></button>
                                        <a :href="route('web.transactions.export-pdf', trx.raw_id)" target="_blank" class="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg"><Download class="w-4 h-4" /></a>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
        ...
        <div v-if="isModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" @click.self="closeViewModal">
            <div class="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
                <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div class="flex items-center gap-3">
                        <div :class="selectedTransaction?.type === 'In' ? 'bg-emerald-100 text-emerald-600' : 'bg-purple-100 text-purple-600'" class="p-2 rounded-xl"><History class="w-5 h-5" /></div>
                        <div>
                    ...
                    <button @click="closeViewModal" class="p-2 text-slate-400 hover:text-red-500 rounded-full transition-colors"><XCircle class="w-5 h-5" /></button>
                </div>
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-1">
                            <p class="text-[9px] text-slate-400 font-black uppercase flex items-center gap-1.5"><Building2 class="w-3 h-3" /> Office / Dept</p>
                            <p class="text-[11px] font-bold text-slate-700 uppercase">{{ selectedTransaction?.department || (selectedTransaction?.type === 'In' ? 'Supplier' : 'General') }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-[9px] text-slate-400 font-black uppercase flex items-center gap-1.5"><User class="w-3 h-3" /> Handled By</p>
                        ...
                        <div class="space-y-1">
                            ...
                            <p :class="selectedTransaction?.type === 'In' ? 'text-emerald-600' : 'text-purple-600'" class="text-lg font-black">{{ selectedTransaction?.type === 'In' ? '+' : '-' }}{{ selectedTransaction?.quantity }}</p>
                        </div>
                        <div class="space-y-1">
                ...
                <div class="p-4 bg-slate-50 border-t border-slate-100 flex gap-2">
                    <a :href="route('web.transactions.export-pdf', selectedTransaction?.raw_id)" target="_blank" class="flex-1 py-3 bg-slate-900 text-white text-[10px] font-black rounded-xl uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-slate-800 transition-all"><Download class="w-4 h-4" /> Download Report</a>
                    <button @click="closeViewModal" class="px-6 py-3 bg-white border border-slate-200 text-slate-500 text-[10px] font-black rounded-xl uppercase hover:bg-slate-100 transition-all">Close</button>
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
